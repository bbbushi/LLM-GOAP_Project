...
M1（GOAP 内核）任务卡 ①②③ 完成：核心接口（六接口 + 伴生 IRuleSet）与 World State JSON Schema v1 已冻结，WorldState 默认实现完成；卡 ③ Action/Goal 配置 Schema v1（`Docs/schemas/action.schema.json`、`goal.schema.json` + 示例）已冻结并实现加载器（`Assets/script/Core/Config/`：零依赖 MiniJson 解析器 + ActionConfig/GoalConfig + ContentLoader），登记见 `Docs/DESIGN.md` §4.4。单测共 76 项全绿（WorldState 32 + MiniJson 22 + 配置加载 22，`Assets/script/Tests/Editor/`；headless mono 反射跑器验证，编辑器 Test Runner 正式留档仍待做，见进行中）。语义基线沿用 `Docs/GOAP.md`（缺失键视为 0、前提 ≥、效果 +=），算子受控泛化（见已决策·M1 核心契约形态）。`Assets/script/Test.cs` 仍为模板占位。协作基础设施：README、`Docs/AGENT_COMMON.md`、/checkpoint、/supervise 监管小组、每日 devlog（`Scripts/devlog/`）、共享远程（提交后即 push）。监管小组首次审查已关闭：对象 DESIGN+HANDOFF，11 条发现全部修复并经独立复核确认无新矛盾，报告在 `Docs/reviews/2026-09-10-design-handoff.md`；`Docs/DESIGN.md` 现 v0.7（§4.4 新增 Action/Goal 配置契约登记行）。

## 进行中

- （可选留档，非阻塞）在 Unity Test Runner（EditMode）跑一遍 Vibe.Core.Tests——headless mono 反射跑器已验证 76/76 全绿（本机 mcs 编译 + 项目 PackageCache 的 nunit.framework.dll），此步仅为正式化记录。注意：headless 复现时需把 nunit.framework.dll 复制到测试 dll 同目录供 mono 解析。
...
1. M1 后续任务卡（按序推进，粒度可再拆；卡号只增不复用，①②③ 已完成见当前状态）：
   - ④ A* Planner 正式实现（二叉堆优先队列 + 签名去重 + 迭代/深度预算）+ 单测；
   - ⑤ tick 循环与资源结算：1 NPC、3 Action、2 Goal 自主生存 24 游戏小时（M1 验收；示例内容可直接取 `Docs/schemas/examples/` 的 3 Action + 2 Goal 扩展成正式 Config）。
