...
- **每日开发日志自动化**（2026-09-08）：launchd 每晚 21:40 用 glm-5.3-flash 无头扫描仓库，生成 `Docs/devlog/YYYY-MM-DD.md`（编年史）并单独提交 `devlog: 日期`，与 HANDOFF.md（工作内存）互补；含本机 `on|off|status` 开关（off=标志文件跳过，不卸载任务，每机自管不入库）。机制与安装见 `Scripts/devlog/README.md`，概览见 AGENT_COMMON.md §5。
...
4. 待办：配置共享远程仓库（GitHub/Gitee 私有），当前本地分支已关联 origin 但协作流程未定。
